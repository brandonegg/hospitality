    ...
    if (status === "success" && data) {
      ...
      for (
        ...
        for (let min = 0; min < 2; min++) {
          if (
            data.startHour % 2 === 1 &&
            hour === ~~(data.startHour / 2) &&
            min === 0
          ) {
            // if the start time is :30 then skip the :00 time
            min = 1;
          }
          if (hour > 11) {
            // pm
            ...
            let tempHour = hour;
            if (hour === 0) {
              // hour 0 is midnight which should be 12:00 am instead of 0:00 am
              tempHour = 12;
            }
      ...
      if (data.endHour % 2 === 1) {
        // if the end time has a remainder, we need the next :00
    ...
    <div className="availabilitySetter flex flex-col items-center justify-center px-2 py-0">
      ...
      {status === "success" &&
        times?.map((time, index) => (
          <AvailButton key={index} text={time} day={day}>
            {" "}
          </AvailButton>
        ))}
    </div>
    ...
    <>
      ...
      <main className="min-h-screen">
        ...
        <div className="availabilitySetter flex flex-row items-center justify-center gap-2 px-2 py-0 ">
          <ColOfAvail day="Sunday" week={weekCount}>
            {" "}
          </ColOfAvail>
          <ColOfAvail day="Monday" week={weekCount}>
            {" "}
          </ColOfAvail>
          <ColOfAvail day="Tuesday" week={weekCount}>
            {" "}
          </ColOfAvail>
          <ColOfAvail day="Wednesday" week={weekCount}>
            {" "}
          </ColOfAvail>
          <ColOfAvail day="Thursday" week={weekCount}>
            {" "}
          </ColOfAvail>
          <ColOfAvail day="Friday" week={weekCount}>
            {" "}
          </ColOfAvail>
          <ColOfAvail day="Saturday" week={weekCount}>
            {" "}
          </ColOfAvail>
        </div>
 ...
 * @returns
 */
export const getServerSideProps: GetServerSideProps<AvailPageProps> = async (
