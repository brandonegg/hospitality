...
export default async function getAvails(
  ...
  const weekCount = (req.body as { docId: string; weekCount: number })
    .weekCount;
  ...
  if (doctorId === "AllDoctors") {
    const result = await prisma.$queryRawUnsafe(
      `SELECT * FROM Availability WHERE date BETWEEN "${dateToString(
        sunday
      )}" AND "${dateToString(saturday)}"`
    );
    return res.status(201).json(result);
  }
  const result = await prisma.$queryRawUnsafe(
    `SELECT * FROM Availability WHERE docId="${doctorId}" AND date BETWEEN "${dateToString(
      sunday
    )}" AND "${dateToString(saturday)}"`
  );
