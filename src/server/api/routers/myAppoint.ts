...
export const removeAppointRouter = createTRPCRouter({
  removeAppoint: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const result: Appointment[] = await ctx.prisma.$queryRawUnsafe(
      ...
      await ctx.prisma.$executeRawUnsafe(
        `INSERT INTO Availability (id, day, startTime, endTime, docId, date) VALUES ("${
          realResult.id
        }", ${realResult.day}, "${realResult.startTime}", "${
          realResult.endTime
        }", "${doctorId}", "${new Date(
          realResult.date.getTime() -
            realResult.date.getTimezoneOffset() * -60000
        )
          .toISOString()
          .slice(0, 10)}")`
      );
