...
export const visitReportRouter = createTRPCRouter({
    ...
    .mutation(async ({ ctx, input }) => {
        ...
        const patientIdMatches = await ctx.prisma.$executeRaw`SELECT id FROM User WHERE id = ${patientId}`;
        const doctorIdMatches = await ctx.prisma.$executeRaw`SELECT id FROM User WHERE id = ${doctorId}`;
