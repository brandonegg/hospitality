...
type TestDeleteInput = RouterInputs["test"]["delete"];
type TestDeleteOutput = RouterOutputs["test"]["delete"];
...
const TestDelete = ({ refetch, test, setPopup }: TestDeleteProps) => {
  ...
  const { mutate } = api.test.delete.useMutation({
    ...
    <form className="flex flex-col gap-2" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="space-y-1">
        <div className="grid grid-cols-10">
          ...
          <p className="col-span-10 sm:col-span-8">{test?.name}</p>
        </div>
        ...
        <div className="grid grid-cols-10">
          ...
          <p className="col-span-10 sm:col-span-8">{test?.description}</p>
        </div>
      </div>
